function [time,best_idv1] = ANN_GA (i,ii)
...
mkdir([pwd,'\ANN+GA-k-fold\'],[currDate '_ANN-GA_',num2str(i)]);

direc = [pwd,'\ANN+GA-k-fold\',currDate '_ANN-GA_',num2str(i)];
...
time = toc
Best_chromosome = [round(chromosome(1:9)),chromosome(10:13)]; % Best Chromosome
...
MseTr = mse(trainT,trainY)
MseTe = mse(testT,testY)

RmseTr = sqrt(MseTr)
RmseTe = sqrt(MseTe)

MaeTr = mae(trainT,trainY)
MaeTe = mae(testT,testY)
...
% regplot(allT,allY,Rall(1,2),RMSE,'ANN-GA','All Data')
...
str=['Model: ANN-GA',newline,'Dataset: All Data',newline,'R = ',...
